    ...
    <script type="module">
        ...
        onSnapshot(q, (querySnapshot) => {
            ...
            documents.forEach(function(document, index) {
                ...
                const incrementButton = index === 0 ? '' : `<button class="increment-btn" data-id="${document.id}"><i class="fa-solid fa-arrow-up"></i></button>`;

                // indexが最後の行でない場合のみdecrementButtonを追加
                const decrementButton = index !== documents.length - 1 ? `<button class="decrement-btn" data-id="${document.id}"><i class="fa-solid fa-arrow-down"></i></button>` : '';
                ...
                tableRows += `
                    <tr style="height: 46px;" class="${rowStyleClass}">
                        ...
                        <td>${convertTimestampToDatetime(document.data.time)}</td>
                        <td>${deleteButton}</td> 
            ...
            //----------------------------------------
            $("#output").off("click", ".decrement-btn").on("click", ".decrement-btn", function() {
                const decrementDocId = $(this).data("id");
                handleDecrementOrder(decrementDocId, documents);
            });
            ...
            $("#output").off("click", ".increment-btn").on("click", ".increment-btn", function() {
                isHandling = true; // 処理中のフラグをtrueに設定
                const incrementDocId = $(this).data("id");
                handleIncrementOrder(incrementDocId, documents);
            });
        ...
        function handleDecrementOrder(decrementDocId, documents) {
            // 対応するドキュメントのデータを取得
            ...
            getDoc(decrementDocRef)
                .then((decrementDoc) => {
                    ...
                    // 他のドキュメントのプロパティのorderがdata.orderより等しい場合に1増やす
                    documents.forEach((document) => {
                        if (document.id !== decrementDocId && document.data.order === decrementData.order) {
                            document.data.order += 1;
                            console.log("increment ID", document.id, document.data.order);
                            const incrementDocRef = doc(db, "tvcha", document.id); // ドキュメントの参照を取得
        ...
        // 各行の「↑」アイコンをクリックしたときのイベントハンドラ
        function handleIncrementOrder(incrementDocId, documents) {
            // 対応するドキュメントのデータを取得
            ...
            getDoc(incrementDocRef)
                .then((inrementDoc) => {
                    ...
                    documents.forEach((document) => {
                        ...
                        if (document.id !== incrementDocId && document.data.order === incrementData.order) {
                            document.data.order -= 1;
                            // console.log("decrement ID", document.id, document.data.order);
                            const decrementDocRef = doc(db, "tvcha", document.id); // ドキュメントの参照を取得
